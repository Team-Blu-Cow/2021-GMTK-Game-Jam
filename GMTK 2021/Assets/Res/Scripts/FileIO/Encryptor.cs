...
namespace FileIO
{
    public class Encryptor
    {
        public Encryptor(byte[] key, byte[] IV)
        {
            ...
            if (m_key.Length != 32)
            {
                Debug.LogWarning("AES Key invalid length, length = " + m_key.Length + " bytes, length of 32 bytes was expected");
            }

            if (m_iv.Length != 32)
            {
                Debug.LogWarning("AES Initial-Vector invalid length, length = " + m_iv.Length + " bytes, length of 32 bytes was expected");
            }
        }
